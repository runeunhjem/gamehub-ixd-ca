...
.map((game) => {
  ...
  return `
  <div class="container game-cards" id="gameCardsContainer">
...
filterSelectElement.addEventListener('change', () => {
  ...
  const filteredGameElements = Array.from(gameElements).filter(gameElement => {
    // replace this logic with your own filtering criteria
    const platformFilter = gameElement.getAttribute("data-platform") === selectedFilter;
    const typeFilter = gameElement.getAttribute("data-type") === selectedFilter;
    return platformFilter || typeFilter;
    return gameElement.getAttribute('container') === selectedFilter;
  });
...
// numberOfGamesDiv.textContent = `Number OF games: ${numberOfFilteredGames}`;
